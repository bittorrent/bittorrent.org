...
``piece layers``
  A list of strings. The list contains one string for each file in the ``file tree``.
  Each string consists of concatenated hashes of one merkle tree layer.
  ...
  A torrent is not valid if this field is absent or the contained hashes do not match
  the merkle roots.
...
``pieces root``
  ...
  As of version 1 **TODO** is used as digest function for the merkle tree.
...
The infohash is calculated by applying **TODO** to the bencoded form of the info dictionary,
...
with either a piece or reject messages. If an unsolicited request
