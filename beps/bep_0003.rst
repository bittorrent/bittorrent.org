...
``piece length``
  ...
  Files are mapped into this piece address space so that each non-empty is
...
``file tree``
  ...
  Keys that contain ``/`` or a null character are reserved and do not represent entries in
  the file tree since they do not represent valid path components on most platforms.
  Keys that exactly match ``.`` or ``..`` are reserved and must be ignored since they
  would result in non-unique path representations.
  'UTF-8 encoded' in this context means that if the actual encoding is known at creation time
  it must be converted to UTF-8.
  Keys may contain invalid UTF-8 sequences or characters that are reserved on specific
  filesystems. Implementations must be prepared to map such names to valid alternatives.
...
is identical. During the download thez must also verify that pieces match both piece
