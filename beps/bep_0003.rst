...
``piece length``
  ...
  at a piece boundary and occurs in the same order as in the file tree.
  The last piece of each file may be shorter than the specified piece length.
  
``meta version``
  an integer value, set to 1 to indicate compatibility with the current revision of this
  specification. Future revisions will only increment this value to indicate an incompatible
...
``pieces root``
  ...
  If necessary the last block of the file must be zero-padded to 16KiB.
  ...
  As of ``meta version`` 1 SHA2-256 is used as digest function for the merkle tree.
...
which is a substring of the metainfo file. For ``meta version`` 1 SHA2-256 is used.
...
hashes outside the info dictionary match the ``pieces root`` fields, without them
a torrent cannot be completed and thus is invalid.
...
to the leaf and piece layers. Index is the offset within the base layer
of the first requested hash. Length is the number of hashes to include
from the base layer. Proof layers is the number of ancestor layers to include.
Hash requests MUST be anwered with either a 'hashes' or 'hash reject' message.
...
message. Hashes is in breadth first order starting with the base layer.
Only ancestor hashes required to verify the base layer hashes are included.
...
is identical and also verify that pieces match both piece hash formats.
If any inconsistency is detected they may either abort or fall back to
