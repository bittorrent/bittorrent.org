...
Extensions to the BitTorrent wireline protocol to delegate seeding
or improve cache performance are beyond the scope of this BEP. 
...
one SRV resource record for every querying host.  A natural but
incorrect solution is to use a wildcard of the form::
...
have an exact label match will match the wildcard.  Thus,
